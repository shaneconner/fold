...
async function gateCommitSurfacesTellTheTruth() {
  ...
  // truecolor the cells take the accent ink, the theme's stand-in for the map's last shade.
  ...
  // DOTTED MARKS (Shane 2026-09-06, sixth pass): a full-height one-pixel stroke inside
  // a solid colour reads as the boundary between two folds of that colour; a mark that
  // does not span the cell's height cannot be a boundary. Brackets before that sat three
  // pixels off their lines because a font insets them.
  assert.equal(rendered.split("┊").length - 1, 2, "the two guides must each be drawn exactly once");
  for (const glyph of ["▏", "▕", "[", "]", "┆"]) assert(!rendered.includes(glyph), `a ${glyph} guide returned`);
  ...
  // Without truecolor the theme's inks are the fill, so BOTH marks stand muted on the
  // default background; with truecolor (below) the one over the fill takes the toned ink.
  assert.equal(weighted.split("<M>┊</M>").length - 1, 2, `the two marks are not muted without truecolor: ${weighted}`);
  assert(!weighted.includes("<B>┊"), "a guide took bold");
  // The aim stands in the column whose LEFT edge is the aim share and the commit in the
  // column whose RIGHT edge is the commit share: each guide in the band's own edge cell
  ...
  assert(solidTrack.includes("\x1b[48;2;53;54;54m\x1b[39m<M>┊</M>\x1b[49m"), `the commit mark on the track does not stand on the track's shade: ${solidTrack}`);
  ...
  // Body and label ink must agree, including pins and pending consolidations. All six
  // shades are the default palette's own, read through the runtime's shade function
  // (gate 171 owns what that function promises); the pin is the map's LAST shade (Shane
  // 2026-09-06, second pass: it sat in the accent for one build and came back onto the
  // map at the far end, the most settled state at the end of the ramp).
  const colours = {
    dark: order.map((kind) => context.foldBarShades(context.DEFAULT_FOLD_BAR_PALETTE, true)[kind]),
    light: order.map((kind) => context.foldBarShades(context.DEFAULT_FOLD_BAR_PALETTE, false)[kind]),
  };
  ...
  for (const [name, palette] of Object.entries(colours)) {
    ...
    for (const [word, index] of [["1 Cons.", 0], ["1 Span", 1], ["1 Tool", 2], ["1 Mark", 3], ["1 Pin", 5]]) {
    ...
    assert(text.includes(over(aimUnder, "┊")), `the ${name} aim mark is not the toned ink over its own kind's background: ${text}`);
    ...
    assert(context.renderFoldBar(deep, 400, theme).includes(over(palette[5], "┊")), `the ${name} commit mark over pale pink is not drawn in the toned ink`);
    ...
    for (const hex of palette) {
      ...
      assert((Math.max(bg, fg) + 0.05) / (Math.min(bg, fg) + 0.05) >= 4.5,
...
async function gateFoldCompositionCountsVisibleMass() {
  ...
  const note = { type: "custom_message", id: "diagram-note", parentId: built.entries[gapAt - 1].id,
    customType: "test-note", display: false, content: "A custom note in a raw gap belongs to the pending parent.",
    timestamp: "2026-09-05T00:00:00.000Z" };
  built.entries.splice(gapAt, 0, note);
  built.entries[gapAt + 1].parentId = note.id;
  ...
  built.snapshot = context.mapActiveContext({ sessionId: built.sessionId,
    eventMessages: built.messages, contextEntries: built.entries, contextWindow: 1_000_000, thresholds: TINY_FOLD_FLOOR });
  let state = context.emptyActiveContextState(built.sessionId);
  for (const turn of [0, 1, 3, 5, 7, 9]) {
    ...
    state = (await commitCandidate(state, built.snapshot, context.manualFoldCandidate(built.snapshot, state, ids), {
      brief: `Chapter ${turn} stays independently addressable beneath a future parent.`, now: turn + 1,
    })).state;
  }
  ...
  assert.equal(roots[0].end + 1, roots[1].start, "the endpoint fixture has no adjacent roots");
  const parents = context.consolidationMarks({ state, snapshot, ordinal: 1 });
  assert.equal(parents.length, 1, "the fixture owes no pending parent");
  const parent = parents[0];
  assert(parent.parts.some((part) => part.kind === "raw" && part.ref.entryId === note.id),
    "the fixture did not include the custom-message gap");
  state = context.withPendingMarks(state, parents);
  const runtime = makeRuntime(built, { thresholds, initialEntries: [
    ...built.entries, stateEntry(built.sessionId, state, "diagram-state", built.entries.at(-1).id),
  ] });
  runtime.ctx.sessionManager.buildSessionContext = () => ({ messages: runtime.branch.flatMap(context.sessionEntryMessages) });
  let widget;
  runtime.ctx.ui.theme = { fg: (_c, t) => t, bold: (t) => t, getColorMode: () => "256color" };
 ...
 * should choose and explore. The bar now reads its six shades, consolidated through
 * pinned, off a named colour map between a start and an end position, and three rows on
 * /fold-settings drive it with the status line as the preview. (The pin sat off the ramp
 * in the theme accent for one build; Shane put it back at the far end the same day.)
 *
...
async function gateFoldBarPaletteIsAReadableChoice() {
  ...
  const ramp = [...context.FOLD_BAR_KINDS];
  ...
  assert.equal(backward.consolidated, forward.pinned);
  assert.equal(backward.pinned, forward.consolidated);
  assert.equal(backward.tool, forward.marked);
  ...
  try {
    ...
    assert(/Bar colours\s+magma/.test(rows()) && /Folded shade at\s+20% along the map/.test(rows()) && /Pinned shade at\s+90% along the map/.test(rows()),
    ...
    // The pinned end is at 0.9; stepping the folded end up from 0.2 lands on 0.85 then
    ...
    screen.handleInput("\x1b[C");
    assert.equal(widget.model.palette.start, 0.95, "the folded end stepped onto the pinned end");
    ...
    assert(/Pinned shade at\s+end of the map/.test(rows()), rows());
