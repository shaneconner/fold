...
Two hairline guides sit at the exact shares they name, over whatever colour the fill holds there: `maxTarget`, where automatic folding is triggered, in bold text ink, and `minTarget`, where a commit aims to land, dimmed. Neither takes a palette shade or a warning colour. Before a measurement the row says `not measured yet`; after a commit it identifies the old reading until the provider measures again. Crossing the target is routine runtime work, not a user action item. On reload, saved session context supplies the fold counts and composition without a new model message; the next real context event replaces that read-only view. Unavailable composition is stated as `mapping`; if folding stops, `FOLDING STOPPED` replaces the bar. Non-truecolor terminals use the theme's own colours.
...
| `palette` | `{ map: "batlow", start: 0, end: 1 }` | The fold bar's colours, and nothing about folding. `map` is one of `batlow`, `viridis`, `plasma`, `magma`, `cividis`; `start` and `end` are positions on it in `[0, 1]` for the most compressed and the raw shade, which may not coincide and may run backwards. Set whole or not at all. |
